...
/* DRIVER FUNCTION */
bool solvek(ll k, vp v)
{
    ll lo = 0, hi = 0;
    for (auto &ele : v)
    {
        if (lo - k > ele.second || hi + k < ele.first)
        {
            return false;
        }
        lo = max(lo - k, ele.first);
        hi = min(hi + k, ele.second);
    }
    return true;
}
...
void solve()
{
    ...
    // Using binary search to find minimum value of k
    ll lo = 0, hi = 1e9, mid = lo + (hi - lo) / 2, ans;
    while (lo <= hi)
    {
        if (solvek(mid, v))
        {
